...
describe("Testing <App logOut={function} />", () => {
  ...
  it('displays the notification drawer when the state is true', () => {
    ...
    expect(wrapper.find('Notification').props().displayDrawer).toBe(true);
  });

  it('hides the notification drawer when the state is false', () => {
    ...
    expect(wrapper.find('Notification').props().displayDrawer).toBe(false);
  });
  ...
  it('renders the course list when the user is logged in', () => {
    // Update the user state to simulate a logged-in user
    store.dispatch({ type: 'LOGIN', email: 'test@example.com', password: 'password' });
